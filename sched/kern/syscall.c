...
// Returns environment's priority on success or -1 if
// environment envid doesn't currently exist.
static int
sys_get_priority(envid_t envid)
{
	struct Env *thisenv;
	int r = envid2env(envid, &thisenv, false);

	if (r < 0)
		return -1;

	return thisenv->priority;
}
...
sys_exofork(void)
{
	...
	newenv->priority = sys_get_priority(newenv->env_parent_id);
	newenv->time_slices = 0;
	add_new_env_to_history(newenv);
...
// Increases an environment's priority.
//
// Returns 0 if env's priority was increased correctly, or -1
// on error. Errors are:
//
//		An environment can't increase it's own priority.
//		Child can't increase it's parent priority.
//		Environment envid doesn't currently exist.
static int
sys_inc_priority(envid_t envid)
{
	struct Env *thisenv;
	int r = envid2env(envid, &thisenv, false);
	...
	if (thisenv == curenv || thisenv->env_parent_id == curenv->env_id) {
		return -1;
	}

	if (thisenv->priority != 1)
		thisenv->priority -= 1;
	return 0;
}

// Reduces an environment's priority.
//
// Returns 0 if env's priority was reduced correctly, or -1
// on error. Errors are:
//
//		Environment envid doesn't currently exist.
static int
sys_red_priority(envid_t envid)
{
	struct Env *thisenv;
	int r = envid2env(envid, &thisenv, false);
...
syscall(uint32_t syscallno, uint32_t a1, uint32_t a2, uint32_t a3, uint32_t a4, uint32_t a5)
{
	...
	case SYS_yield:
		sys_yield();  // No return
	case SYS_inc_priority:
		return sys_inc_priority(a1);
	case SYS_red_priority:
		return sys_red_priority(a1);
