...
expand_environ_var(char *arg)
{
	...
	if (env_len > sizeof(arg)) {
		arg = (char *) realloc(arg, env_len + 1);
		if (!arg) {
			_exit(-1);
		}
	}
	memcpy(arg, env, env_len);
	return arg;
}
...
// if the command contains multiple '|' characters it
// recursively calls itself to parse the right part
// of the split
struct cmd *
parse_line(char *buf)
{
